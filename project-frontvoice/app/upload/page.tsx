...
export default function UploadPage() {
  ...
  // เก็บรายการไฟล์ที่อยู่ในคิวอัปโหลด (อ่านค่า: บรรทัด 47, 91, 94, 95, 133, 134, 185, 189, 195, 246; อัปเดต: บรรทัด 57, 62, 99, 120, 124)
  const [queue, setQueue] = useState<QueuedFile[]>([]);
  // เก็บสถานะว่าผู้ใช้กำลังลากไฟล์อยู่บน drop zone หรือไม่ (อ่านค่า: บรรทัด 150, 164, 169; อัปเดต: บรรทัด 68, 74, 80)
  const [isDragging, setIsDragging] = useState(false);
  // เก็บสถานะกำลังประมวลผล/อัปโหลดไฟล์ในคิว (อ่านค่า: บรรทัด 91, 246, 249; อัปเดต: บรรทัด 92, 130)
  const [isProcessing, setIsProcessing] = useState(false);

  // แปลงขนาดไฟล์จาก bytes เป็น KB/MB สำหรับแสดงใน queue (ถูกเรียกใช้ที่บรรทัด 53)
  const formatSize = (bytes: number) => {
    if (bytes < 1024 * 1024) return `${(bytes / 1024).toFixed(0)} KB`;
    return `${(bytes / (1024 * 1024)).toFixed(1)} MB`;
  };

  // Handle: เพิ่มไฟล์ที่เลือกหรือ drag/drop เข้าคิว upload (ถูกเรียกใช้ที่บรรทัด 81, 161)
  ...
  // Handle: ลบไฟล์ออกจากคิว upload (ถูกเรียกใช้ที่บรรทัด 224)
  const removeFile = (id: string) => {
    setQueue(prev => prev.filter(f => f.id !== id));
  };

  // Handle: จัดการตอนลากไฟล์อยู่เหนือ drop zone (ถูกเรียกใช้ที่บรรทัด 145)
  const handleDragOver = (e: DragEvent) => {
    e.preventDefault();
    setIsDragging(true);
  };

  // Handle: จัดการตอนลากไฟล์ออกจาก drop zone (ถูกเรียกใช้ที่บรรทัด 146)
  const handleDragLeave = (e: DragEvent) => {
    e.preventDefault();
    setIsDragging(false);
  };

  // Handle: รับไฟล์จากการ drop แล้วเพิ่มเข้าคิว upload (ถูกเรียกใช้ที่บรรทัด 147)
  ...
  // Handle: เปิด file picker จากปุ่มเลือกไฟล์ (ถูกเรียกใช้ที่บรรทัด 148, 175)
  const handleSelectFiles = () => {
    fileInputRef.current?.click();
  };

  // Handle/API: อัปโหลดไฟล์ในคิวไป backend ทีละไฟล์และอัปเดตสถานะใน queue (ถูกเรียกใช้ที่บรรทัด 245)
    ...
    <div className="flex h-screen bg-white dark:bg-slate-900 overflow-hidden"> {/* Layout หลักหน้า Upload: sidebar + upload workspace */}
      ...
      <main className="flex-1 p-8 overflow-auto"> {/* พื้นที่ทำงานสำหรับเลือกไฟล์และจัดคิว upload */}
        ...
        <div className="flex gap-8"> {/* แบ่งหน้าจอเป็น drop zone ซ้าย และ queue ขวา */}
          ...
          <div
            ...
            className={`flex-1 border-2 border-dashed rounded-[32px] flex flex-col items-center justify-center p-10 min-h-[420px] transition-all cursor-pointer group ${
              isDragging
                ...
                : 'border-slate-200 bg-slate-50/30 dark:bg-slate-800/30 hover:bg-slate-50 dark:hover:bg-slate-700 hover:border-blue-200'
            }`}
          >
            <input
              ref={fileInputRef}
            ...
            <p className="text-slate-400 dark:text-slate-500 text-xs mb-8 text-center max-w-[280px] leading-relaxed">
              Maximum 60mb. Supported formats: <span className="text-slate-500 dark:text-slate-400 font-medium">{SUPPORTED_FORMATS.join(', ')}</span>
            </p>
            <button
          ...
          {/* Right Sidebar - File Queue: แสดงไฟล์ที่รอ upload และผลลัพธ์แต่ละไฟล์ */}
          <div className="w-80 flex flex-col">
            <h2 className="text-lg font-medium text-slate-800 dark:text-slate-100 mb-4">
              File Queue {queue.length > 0 && <span className="text-sm text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">({queue.length})</span>}
            </h2>

            <div className="space-y-2 mb-6 max-h-[350px] overflow-y-auto pr-2"> {/* รายการ queue: pending/uploading/done/error */}
              {queue.length === 0 ? (
                ...
                queue.map((item) => (
                  <div key={item.id} className={`flex items-center p-2.5 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-xl shadow-sm transition-colors ${
                    ...
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center mr-3 shrink-0 ${
                      ...
                      'bg-slate-50 dark:bg-slate-700 text-blue-600 dark:text-blue-400'
                    }`}>
                    ...
                    {item.status === 'pending' && (
                      <button
                        ...
                        className="text-slate-300 dark:text-slate-600 hover:text-slate-500 dark:hover:text-slate-400 p-1 cursor-pointer shrink-0"
                      >
                        <X size={14} />
                      </button>
                    )}
            ...
            {allDone ? ( /* ถ้า upload สำเร็จครบทุกไฟล์ ให้พาไปดูหน้า Files */
              <button
            ...
            ) : ( /* ถ้ายังไม่ครบ ให้แสดงปุ่มเริ่ม upload */
              <button
            ...
            <p className="text-[10px] text-slate-400 dark:text-slate-500 text-center mt-3 px-4">
